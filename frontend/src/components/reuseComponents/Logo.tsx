...
export const Logo = ({ onClick }: LogoType) => {
  return (
    <button onClick={onClick} className="w-28 sm:w-32 md:w-40 cursor-pointer flex items-center gap-1 hover:opacity-80 transition-opacity">
      ...
      <div className="relative h-10 w-32 hidden lg:block">
        <Image src={LOGO_TEXT} alt="LOGO_TEXT" fill className="object-contain" />
      </div>{" "}
    </button>
  );
};

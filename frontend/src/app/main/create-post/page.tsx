...
const CreatePost = () => {
  ...
  const fetchBoards = useCallback(async (search: string = "") => {
    ...
    try {
      ...
      if (response.data) {
        const filteredBoards = response.data.filter((board:any) => board.isActive !== false);
                  ...
                  {showDropdown && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg max-h-[min(320px,60vh)] overflow-y-auto z-50">
                      ...
                      <div className="border-b border-gray-100 px-2 py-1">
                        <p className="px-2 py-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">{t("categoriesLabel")}</p>
                      ...
                      <div className="px-2 py-1">
                        <p className="px-2 py-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">{t("communitiesLabel")}</p>
                        {isLoadingBoards ? (
                          <Loading />
                        ) : boards.length > 0 ? (
                          boards.map((board) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {board.description && <p className="text-xs text-gray-500 truncate">{board.description}</p>}
                              </div>
                            </button>
                          ))
                        ) : (
                          <div className="p-4 text-center text-gray-500 text-sm">{searchQuery ? t("noBoardsFound") : t("noBoardsAvailable")}</div>
                        )}
                      </div>
                    </div>
                  )}
